  ...
  return (
    <>
      ...
      <ScrollView style={[styles.screen, { backgroundColor: palette.background }]} contentContainerStyle={styles.content}>
        <SectionCard title={topic?.title ?? String(topicSlug)} subtitle={topic?.description ?? 'Prayer-focused narrations'}>
          {items.map((item) => (
            <View key={item.id} style={[styles.card, { borderColor: palette.border, backgroundColor: palette.surface }]}>
              <Text style={[styles.meta, { color: palette.subtleText }]}>{item.bookSlug} #{item.bookHadithNumber}</Text>
              ...
              {item.explanation ? <Text style={[styles.english, { color: palette.subtleText }]}>{item.explanation}</Text> : null}
