...
export default function HadithChapterScreen() {
  ...
  useEffect(() => {
    if (!isReady || !bookSlug || Number.isNaN(parsedChapterId)) {
      return;
    }
          ...
          {items.length ? (
            items.map((item) => (
              <View key={item.id} style={[styles.card, { borderColor: palette.border, backgroundColor: palette.surface }]}>
                ...
                <Text style={[styles.english, { color: palette.text }]}>
                  {item.textEnglish || 'No English text in source record.'}
                </Text>
                {item.explanation ? (
                  <Text style={[styles.explanation, { color: palette.text }]}>{item.explanation}</Text>
                ) : null}
            ...
            <Text style={[styles.meta, { color: palette.subtleText }]}>No narrations available in this chapter.</Text>
          )}
        </SectionCard>
      </ScrollView>
    </>
...
const styles = StyleSheet.create({
  ...
  card: { borderWidth: 1, borderRadius: 16, gap: 9, padding: 14 },
  ...
  button: { alignItems: 'center', borderRadius: 12, borderWidth: 1, paddingVertical: 10 },
